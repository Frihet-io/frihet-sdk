...
3. **Tag the merge commit, annotated.** Lightweight tags are rejected by the
   *Assert the tag is annotated* step:
   ```sh
...
| 3 | Tag authority: annotated tag, on a `main` ancestor, matching both manifests | *Assert the tag is annotated* · *Assert the tagged commit is an ancestor of origin/main* · *Assert both package versions equal the tag* | All three run in `verify`, before any job that can reach the environment or a credential |
| 4 | No republish, ever; no `--force` | *Assert neither version is already published* in `verify`, then *Re-assert @frihet/sdk is not already published* / *Re-assert frihet is not already published* inside each publish job (a **zero** exit from `npm view` is the failure) | A re-pushed tag stops in `verify`; a GitHub "re-run failed jobs", which reuses `verify`'s cached success, stops inside the publish job before `npm publish` runs |
...
| 10 | Least privilege | `permissions: {}` at workflow level; per-job `permissions:`; `id-token: write` only on `publish-sdk` and `publish-cli`; `concurrency: publish-${{ github.ref }}`, `cancel-in-progress: false` | `verify` and `post-publish` never hold an OIDC credential; two releases cannot interleave and a release cannot be cancelled between the two publishes |
